...
const SliderValidation = () => {
  ...
  const initCavas = (x, y) => {
    const canvas = document.getElementById('picture-slider-canvas') as HTMLCanvasElement;
    ...
    img.src = 'https://fastly.picsum.photos/id/1075/400/180.jpg?hmac=vArTvf0OXBA8X1v9xfC-23-OMx4NvPMY_ZiYYQhqL0E' ;
    const loadCount = 0;
    img.onload = () => {
      ...
      const imgData = blockCtx?.getImageData(randomX-3, randomY, L, L) as ImageData;
      block.width = L;
  ...
  useEffect(() => {
    ...
    const picture = document.getElementById('picture-slider') as HTMLInputElement;
    const pictureBorder = document.getElementById('picture-slider-border') as HTMLInputElement;
    if (picture) {
    ...
    if (slider) {
      ...
      document.addEventListener('mouseup', () => {
        ...
        const leftBorder = Number(pictureBorder?.style?.left?.split('px')?.[0] ?? 0)
  ...
  return (
    <div className={styleCtx('slider-validation-container')}>
      <canvas className={styleCtx('slider-validation-picture')} id='picture-slider-canvas'></canvas>
      ...
      <div className={styleCtx('slider-validation-slider')} key='slider-container-key' id='slider-container'>
        <div
          key='slider-div'
          id='slider'
