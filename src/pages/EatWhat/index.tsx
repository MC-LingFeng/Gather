...
const DefaultMerchant = ['老太太拌饭', '烤面筋卷饼','功夫一锅', '隆江猪脚饭', '吉', '啃得', '麦当儿', 'begerKin','喝粥', '烤鸭', '金手勺']
...
const EatWhat = () => {
  ...
  const interval = ({ times, intervalTime, foodDoms}: {times: number, intervalTime: number, foodDoms: NodeListOf<Element> }) => {

    inter = setInterval(() => {
      
      if (times * 1000 - (intervalIndex * intervalTime) <= 2000 && inter !== null){
    ...
    <div>
      <ul>
        {DefaultMerchant.map((item, index) => {
          return <li data-id={index + 1} key={`${index}--${item}`} className='food-item'>{item}</li>
        })}
      </ul>
      <Button onClick={onClick} loading={loading}>{loading? '正在选择' : '开始'}</Button>
    </div>
  )
}
