...
const ULucky = () => {
  ...
  const data = useRequest(service.setmessage, {
    manual: true,
    onSuccess(res) {},
  });
  const data4 = useRequest(service.setmessage4, {
    manual: true,
    onSuccess(res) {
      console.log(res);
    },
  });
  const imgData = useRequest(service.setmessageimg, {
    manual: true,
    onSuccess(res) {},
  });
        ...
        <Form.Item name="img" label='描述一个图片'>
          ...
          <Form.Item>
            <Button
              ...
              htmlType="submit"
              loading={
                data.loading || imgData.loading || data4.loading || loading
              }
            >
              生成文章！
            </Button>
