...
const useListenScroll: (className: string, addclass: string) => void = (className, addclass) => {
  const scrollFun: (this: Window, ev: Event) => any = () => {
    const listenScrollTop = document.scrollingElement?.scrollTop ?? 0;

    const classDom = document.documentElement.getElementsByClassName(className)[0]
